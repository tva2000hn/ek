...
var numberu = Math.floor(Math.random()*6) + 1;
...
var GameRoom = {
    ...
    updateCardDisplay: function(EK) {
        ...
        $.each($C.CARD, function(key, type) {
            if (type != $C.CARD.EXPLODE) {
                var html = "<div data-selected='false' data-type='" + type + "' class='card noselect card-" + type.toLowerCase() + "'" + "style='background-image:url(https://github.com/tva2000hn/ek/blob/master/" + type.name + numberu + ".jpg?raw=true);background-size:'cover''" + ">" +
                                "<span>" + type + "</span>" +
                            "</div>";
    ...
    updateCardsForElement: function(cards, element) {
        ...
        $.each(cards, function(index, card) {
            var html = "<div data-selected='false' data-id='" + card.id + "' class='card noselect card-" + card.type.toLowerCase() + "'" + "style='background-image:url(https://github.com/tva2000hn/ek/blob/master/" + card.name + numberu + ".jpg?raw=true);background-size:'cover''" + ">" + 
                            "<span>" + card.name + "</span>" +
                        "</div>";
